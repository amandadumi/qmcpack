...
std::unique_ptr<WaveFunctionComponent> JastrowBuilder::buildSNA(xmlNodePtr cur)
{
  ReportEngine PRE(ClassName, "addSNA(xmlNodePtr)");
  std::unique_ptr<SNAJastrowBuilder> sjb;
  auto pa_it(ptclPool.find(sourceOpt));
  sjb = std::make_unique<SNAJastrowBuilder>(myComm, targetPtcl, *pa_it->second);
