...
#include "qmcpack_compute_snap.h"
#include <cstring>

#include "qmcpack_sna.h"

using namespace qmcplusplus;

enum{SCALAR,VECTOR,ARRAY};

ComputeSnap::ComputeSnap(ParticleSet& P, int narg, char **arg) :
  ...
  snaptr(nullptr)
{
  ...
  std::vector radelem(ntypes+1); // offset by 1 to match up with types
  //memory->create(wjelem,ntypes+1,"snap:wjelem");
  std::vector wjelem(ntypes+1);  // offset by 1 to match up with types
  rcutfac = atof(arg[3]);
  rfac0 = atof(arg[4]);
  twojmax = atoi(arg[5]);
  ...
  snaptr = new SNA(&P, rfac0, twojmax,
...
void ComputeSnap::compute_array()
{
  ...
  // AD: resize array if neighbor llist changes
  if (atom->nmax > nmax) {
    memory->destroy(snap_peratom);
    nmax = atom->nmax;
 ...
 // accumulate forces to global array
  ...
  // AD: not ready to think about this yet.
  //MPI_Allreduce(&snap[0][0],&snapall[0][0],size_array_rows*size_array_cols,MPI_DOUBLE,MPI_SUM,world);

  // assign energy to last column

  int irow = 0;
  double reference_energy = c_pe->compute_scalar(); // this is from lammps, we shouldn't need this
