...
{
/// Constructor.
DMCWithHistory::DMCWithHistory(const ProjectData& project_data,
         MCWalkerConfigurationHistory& w,
         TrialWaveFunction& psi,
         QMCHamiltonian& h,
         Communicate* comm,
         bool enable_profiling)
    : QMCDriver(project_data, w, psi, h, comm, "DMCWithHistory", enable_profiling),
      KillNodeCrossing(0),
      BranchInterval(-1),
      L2("no"),
      Reconfiguration("no"),
      ...
      Nhist(10)
{
  ...
  m_param.add(Nhist, "nhist");

}

void DMCWithHistory::resetUpdateEngines()
{
  ReportEngine PRE("DMCWithHistory", "resetUpdateEngines");
...
#pragma omp parallel for
  ...
  }
  {
    ...
    o << "  Number of timesteps back stored = " << nHist << "\n";
    app_log() << o.str() << std::endl;
  }
  app_log() << "  DMCWithHistory Engine Initialization = " << init_timer.elapsed() << " secs" << std::endl;
}

bool DMCWithHistory::run()
{
  LoopTimer<> dmc_loop;
...
#pragma omp for nowait
    ...
    {
      if (!myComm->rank())
        app_log() << runtimeControl.generateStopMessage("DMCWithHistory", block - 1);
...
bool DMCWithHistory::put(xmlNodePtr q)
{
  BranchInterval = -1;
  ParameterSet p;
